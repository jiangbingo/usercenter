...
TEMPLATE_FILE_BIN = u"""\
{
    ...
    "layStatus": "%(layStatus)s",

    "furnSet": "%(furnSet)s",

    "isPlatform": %(isPlatform)s,
    ...
    for line in fileinput.input(fname):
        ...
        if len(chunks) > 0:
            ...
            if chunks[0] == "Ks" and len(chunks) == 4:
                materials[identifier]["colorSpecular"] = [0.2 if float(chunks[1]) == 0 else float(chunks[1]),
                                                          0.2 if float(chunks[2]) == 0 else float(chunks[2]),
                                                          0.2 if float(chunks[3]) == 0 else float(chunks[3])]
    ...
    for line in fileinput.input(fname):
        ...
        if len(chunks) > 0:
            ...
            if chunks[0] == "v" and len(chunks) == 4:
                ...
                vertices.append([x * 0.01 , y * 0.01, z * 0.01])
...
def generate_materials(mtl, materials):
    ...
    for m in mtl:
        ...
        if m in materials:
            index = materials[m]
            # shading = 'basic' if m == 'shade' else 'phong'
            ...
            # mtl[m]['shading'] = shading

            if m == 'glass':
                mtl[m]['transparency'] = 0.5 if float(mtl[m]['transparency']) == 0 else mtl[m]['transparency']
...
def convert_ascii(infile, morphfiles, colorfiles, outfile):
    ...
    n_faces = len(faces)
    coor = bbox(vertices)
    coordinate = str(coor['x'][1]-coor['x'][0]) +","+ str(coor['z'][1]-coor['z'][0]) +","+ str(coor['y'][1]-coor['y'][0])
...
def convert_binary(infile, outfile, args):
...
def createjs(opts, args):
    ...
    if TYPE == "ascii":
        ...
        return convert_binary(infile, outfile, args)
# #####################################################
# Main
# #####################################################
if __name__ == "__main__":
    ...
    if TYPE == "ascii":
        ...
        convert_binary(infile, outfile, args)
